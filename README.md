...
- **면접관 응답 → 충돌 최소 시간 추천 → 후보자 자동 안내**: 패널 전원이 응답 링크로 제출을 마치면, 그 가용 시간 데이터를 바탕으로 전원 동시 가능한 시간을 계산해 곧바로 후보자에게 자동 안내한다. 완전히 겹치지 않는 시간은 개수 제한 없이 전부 후보자에게 보여주고(하나도 없으면 충돌이 가장 적은 시간들을 최대 3개까지 대안으로), 후보자는 그중 하나를 확인하고 확정만 하면 된다. 이 목록은 후보자가 응답 페이지를 열 때마다 그 순간의 면접관 가용 시간으로 다시 계산되므로, 응답을 미루는 동안 일정이 바뀌어도 항상 최신 상태가 반영된다.
- **후보자 응답 → 자동 매칭 → 확정 알림**: 후보자가 제안된 시간을 확정하면 그 자리에서 매칭이 실행되고, 리크루터가 따로 "확정 메일 발송"을 누르지 않아도 후보자·면접관 전원에게 곧바로 확정 안내(문제 있으면 알려달라는 안내 포함)가 나간다. 공통 시간이 없으면 사유와 함께 에스컬레이션된다.
...
- **수동 확정(히트맵)**: 자동 매칭이 안 되거나(전원 공통 시간 없음) 그냥 직접 정하고 싶을 때, 면접 상세 화면에서 시간대별 충돌 인원 수를 히트맵으로 보고 원하는 시간을 직접 골라 확정할 수 있다.
...
- Vercel Cron — 매일 1회 리마인더 배치 실행 (`vercel.json`)
