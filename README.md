...
- **후보자 우선순위 제출 → 리크루터 최종 확정 → 확정 알림**: 후보자는 편한 순서대로 최대 3개까지 우선순위를 골라 제출한다(단일 확정이 아니라 순위제라, 1순위가 그 사이 안 되게 되어도 2·3순위로 버틸 여지가 있다). 리크루터가 케이스 상세에서 순위 중 하나를 클릭해 확정하면 그 자리에서 다시 한 번 실제로 비어있는지 검증하고, 확정되면 후보자·면접관 전원에게 곧바로 확정 안내 메일(문제 있으면 알려달라는 안내 포함)이 나간다 — 리크루터가 따로 "확정 메일 발송"을 누를 필요가 없다.
...
`created` → `interviewer_pending` → `interviewer_done` → `candidate_pending` → `candidate_done`(후보자가 1~3순위 제출, 리크루터의 최종 확정 대기)
