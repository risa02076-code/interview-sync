...
- **통제 — 정해진 시각에 자동 실행되나, 발송 전 사람 승인 단계를 두는지.** 리마인더·정합성 검사는 Cron으로 정해진 시각에 자동 실행되지만, 가장 위험한 동작(확정 메일 발송)은 자동화하지 않았다 — 매칭 계산까지는 자동으로 되지만 실제 발송은 리크루터가 "확정 메일 발송" 버튼을 직접 눌러야만 나간다(자동 확정 경로도 예외 없이 통일). `EMAIL_SENDING_ENABLED=false` 환경변수 하나로 코드 재배포 없이 전체 발송을 즉시 멈추는 킬 스위치도 뒀다.
- **관측 — 전체 실행 기록(요청·응답·분기·오류)을 화면에서 확인할 수 있는지.** 메일 발송은 성공/실패/사유가 화면(`email_sent_at`, `note`)과 로그(`[email-failed]` 태그)에 모두 남는다. 다만 확정 저장 실패(`confirm_interview` 트랜잭션이 던지는 에러)는 호출부 세 곳 중 두 곳(순위 확정 API, 면접관 전원 응답 후 자동 확정)이 이 에러를 잡지 않고 있어서 실패해도 어디에도 기록이 안 남는 공백이 있었다 — 특히 후자는 대시보드 접근 권한이 없는 면접관이 마주치는 실패라 더 위험했다. 두 경로 모두 에러를 잡아 사유를 노출하도록 고치고(`[confirm-failed]` 로그 태그 추가), 테스트가 아예 없던 해당 로직에 회귀 테스트를 새로 붙였다.
- **이식성 — 워크플로를 내보낸 뒤 비밀값 재설정이 필요한지.** `.env.example`에 필요한 값(Supabase URL·서비스 롤 키, Gmail 계정·앱 비밀번호, 크론 시크릿)이 전부 명시돼 있다. DB 마이그레이션(확정 트랜잭션, 회의실 정원 컬럼)은 "먼저 실행 후 배포"라는 순서 자체를 문서에 못박아뒀다 — 순서를 안 지키면 다른 환경에서 확정이 전부 실패한다.
- **비용 — 실행 단위 과금인가, 노드별 과금인가.** 이 질문이 원래 겨냥한 "실행 횟수당 과금" 모델(n8n·Zapier류)과는 다르게, Vercel·Supabase·Gmail SMTP 무료/정액 티어 조합이라 비용 구조 자체가 단순하다. 다만 실제 도입 시 예상 비용은 이 저장소가 아니라 별도 문서([도입 제안서](https://claude.ai/code/artifact/06d77f75-512b-4b92-8b34-62e6f83bced1))에만 있어서, 코드와 붙어 있지는 않다 — 다섯 기준 중 가장 약하게 반영된 항목이다.
- **데이터 경로 — 외부 모델·호스팅 서버·API 사이의 데이터 이동 경로.** LLM을 안 쓰니 외부 모델로 나가는 경로 자체가 없다. 대신 실제 경로 둘은 알려진 한계로 명시해뒀다 — 데이터베이스(Supabase)는 서버만 서비스 롤 키로 접근하고 브라우저는 직접 접근하지 않으며, 이메일은 회사 인프라가 아닌 개인 Gmail SMTP를 거친다는 것 자체를 리스크로 인식해 "실무 도입 시 AWS SES 전환 필요"라고 적어뒀다. 인증이 없어 URL을 아는 사람은 누구나 후보자 개인정보를 볼 수 있다는 것도 노출 위험으로 이미 문서화돼 있다.
