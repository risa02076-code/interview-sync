...
- **리스크 관리 3종 세트** (`lib/email.ts`, `lib/confirmFromPriorities.ts`):
  ...
  - **발송 성공 여부(`email_sent_at`)를 응답 여부와 분리해 기록** — "재발송" 버튼이
    ...
    (`supabase/migration_email_sent_at.sql`), 면접 상세 페이지에서 면접관별·최종 확인별로
    "○ 발송됨, 응답 대기"(회색)와 "⚠️ 발송 실패"(빨간색, 재발송 버튼도 강조)를 구분해서
    보여준다. 단, 애초에 초대 자체를 아직 시도 안 한 상태(`stage: "created"`, 또는 최종
    확인 요청 레코드가 아직 없음)는 실패로 오인하지 않도록 별도로 걸러낸다.
