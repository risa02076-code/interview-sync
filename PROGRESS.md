...
- **리스크 관리 3종 세트** (`lib/email.ts`, `lib/confirmFromPriorities.ts`):
  ...
  - 발송 실패 가시화 — `sendEmail` 실패 시 `[email-failed]` 태그로 Vercel Logs에 명확히 남음
    (대시보드 UI에 케이스별로 뜨는 건 아직 아님 — 로그 레벨 가시성까지만 구현)
...
1. 이메일 발송 실패를 로그뿐 아니라 케이스별로 대시보드에도 가시화(현재는 Vercel Logs까지만)
