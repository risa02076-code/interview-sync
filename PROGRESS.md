...
- **리스크 관리 3종 세트** (`lib/email.ts`, `lib/confirmFromPriorities.ts`):
  ...
  - 발송 실패 가시화 — `sendEmail` 실패 시 `[email-failed]` 태그로 Vercel Logs에 남고, 확정
    메일·후보자 초대 메일은 실패하면 `interview.note`에도 남아 대시보드에 바로 보임(escalated가
    아니어도 note가 있으면 표시하되, "⚠️"가 붙거나 escalated일 때만 빨간색으로 강조)
...
1. 발송 실패 가시화를 나머지 이메일 함수(면접관 초대, 최종 확인 요청, 재문의)까지 확장
   — 지금은 확정 메일·후보자 초대 메일 두 곳만 커버함
