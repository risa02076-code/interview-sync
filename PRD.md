...
| 3 | 면접관 응답 → 충돌 최소 시간 추천 → 후보자 자동 안내 | 패널 전원이 응답을 마치면, 전원 동시 가능한 시간 중(없으면 충돌이 가장 적은 시간) 하나를 계산해 그 시간으로 곧바로 후보자에게 자동 안내한다. 후보자는 여러 시간 중 고르는 대신, 제안된 시간 하나를 확인·확정한다 |
...
interviews (면접 케이스)
  ...
  recommended_slot   text, null  -- 후보자에게 안내한 충돌 최소 추천 시간 (안내 시점에 고정)
