...
| 3 | 면접관 응답 → 충돌 최소 시간 추천 → 후보자 자동 안내 | 패널 전원이 응답을 마치면, 전원 동시 가능한 시간을 계산해 곧바로 후보자에게 자동 안내한다. 완전히 겹치지 않는 시간은 개수 제한 없이 전부 보여주고(없으면 충돌이 가장 적은 시간들을 최대 3개까지), 후보자가 응답 페이지를 열 때마다 그 순간의 면접관 가용 시간으로 다시 계산한다(응답을 미루는 동안 일정이 바뀌어도 항상 최신 반영) |
| 4 | 후보자 응답 → 자동 매칭 → 확정 알림 | 후보자가 제안된 시간을 확정하면 그 자리에서 패널·회의실 가용성을 대조해 확정하고, 리크루터 개입 없이 후보자·면접관 전원에게 곧바로 확정 안내 메일(문제 시 알려달라는 안내 포함)을 발송한다 |
