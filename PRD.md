...
| 4 | 후보자 우선순위 제출 → 리크루터 최종 확정 → 확정 알림 | 후보자는 편한 순서대로 최대 3개까지 우선순위를 골라 제출한다. 리크루터가 케이스 상세에서 순위 중 하나를 클릭해 확정하면(그 자리에서 아직도 비어있는지 다시 확인), 리크루터가 따로 메일을 보내지 않아도 후보자·면접관 전원에게 곧바로 확정 안내 메일(문제 시 알려달라는 안내 포함)이 발송된다 |
...
`created` (등록됨) → `interviewer_pending` (면접관 응답 대기) → `interviewer_done` (면접관 응답 완료, 후보자 자동 발송 트리거) → `candidate_pending` (후보자 응답 대기) → `candidate_done` (후보자가 1~3순위 제출 완료, 리크루터의 최종 확정 대기)
