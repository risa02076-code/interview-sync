...
| 3 | 면접관 응답 → 충돌 최소 시간 추천 → 후보자 자동 안내 | 패널 전원이 응답을 마치면, 전원 동시 가능한 시간(여러 개면 최대 3개까지, 없으면 충돌이 가장 적은 시간들)을 계산해 곧바로 후보자에게 자동 안내한다. 후보자는 전체 시간대를 다시 취합할 필요 없이, 제안된 시간 중 하나를 확인·확정한다 |
