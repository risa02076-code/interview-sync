 ...
 * 회의실 수정(이름·정원·사용 여부).
 *
 * 이미 쓰인 적 있는 회의실은 삭제하지 않고 active=false("사용 안 함")로 둔다.
 * interviews.room_id가 rooms(id)를 참조하므로(supabase/schema.sql) 애초에 지워지지도
 * 않고, 지난 면접 기록에서 회의실 이름이 사라지는 것도 원하는 동작이 아니다.
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  if (body.name !== undefined) {
    if (!body.name.trim()) {
      return NextResponse.json({ error: "회의실 이름은 비울 수 없습니다." }, { status: 400 });
    }
...
export async function DELETE(_request: Request, { params }: Params) {
  ...
  if (referencing?.length) {
    return NextResponse.json(
      {
        error:
          `이 회의실을 쓰는 면접이 ${referencing.length}건 있어 삭제할 수 없습니다. ` +
          "지난 기록에서 회의실 이름이 사라지기 때문입니다 — 대신 '사용 안 함'으로 두면 " +
          "앞으로의 자동 배정에서만 빠집니다.",
      },
      { status: 409 },
    );
